...
function adminHtml() {
  ...
  function rowHtml(r){return '<tr><td>'+esc(r.device_id||'—')+'</td><td>'+esc(r.license_key)+'</td><td>'+esc(r.plan)+'</td><td>'+esc(r.days_remaining)+'</td><td><span class="pill">'+esc(r.status)+'</span></td><td>'+esc(r.last_check||'—')+'</td><td><button data-action="reset" data-id="'+esc(r.id)+'">Reset</button><button data-action="extend" data-days="1" data-id="'+esc(r.id)+'">+1d</button><button data-action="extend" data-days="3" data-id="'+esc(r.id)+'">+3d</button><button data-action="extend" data-days="7" data-id="'+esc(r.id)+'">+7d</button><button data-action="status" data-status="SUSPENDED" data-id="'+esc(r.id)+'">Suspender</button><button data-action="status" data-status="ACTIVE" data-id="'+esc(r.id)+'">Ativar</button></td></tr>'}
  function reportHtml(r){return '<tr><td>'+esc(r.target_package)+'</td><td>'+esc(r.reports)+'</td><td>'+esc(r.last_report)+'</td></tr>'}
  async function load(){try{const j=await api('/api/admin/devices');document.getElementById('rows').innerHTML=j.rows.map(rowHtml).join('');const rr=await api('/api/admin/reports');document.getElementById('reports').innerHTML=rr.rows.map(reportHtml).join('');document.getElementById('msg').textContent='Atualizado.';}catch(e){document.getElementById('msg').textContent=e.message}}
  document.addEventListener('click',async e=>{const b=e.target.closest('button[data-action]');if(!b)return;try{const id=b.dataset.id;const action=b.dataset.action;if(action==='reset')await api('/api/admin/license/reset',{method:'POST',body:JSON.stringify({id})});if(action==='extend')await api('/api/admin/license/extend',{method:'POST',body:JSON.stringify({id,days:Number(b.dataset.days)})});if(action==='status')await api('/api/admin/license/status',{method:'POST',body:JSON.stringify({id,status:b.dataset.status})});await load();}catch(err){document.getElementById('msg').textContent=err.message}});
  async function createLicense(){try{const j=await api('/api/admin/license/create',{method:'POST',body:JSON.stringify({plan:document.getElementById('plan').value})});document.getElementById('msg').textContent='Nova licença: '+j.license_key;await load()}catch(e){document.getElementById('msg').textContent=e.message}}
</script></body></html>`;
}
